...
{
    // 获取Email节点
    NSXMLElement *emailElement = [self elementForName:@"EMAIL"];
    
    // 获取Email的USERID子节点
    NSXMLElement *useridElement = [emailElement elementForName:@"USERID"];
    
    NSString *email = [useridElement stringValue];
    if (email.length > 0) {
        return @[email] ;
    }else{
        return nil;
...
{
    // 邮箱数组只能有一个邮箱
    // 下面代码的意思是只有当 "emails.count == 1" 这种情况时才会继续执行下面的代码,
    // 否则"emails.count != 1"当会导致程序崩溃并抛出异常,log输出"emails.count == 1"
    ...
    // 先删除USERID节点
    [emailElement removeElementForName:@"USERID"];
    
    // 载添加回USERID节点
    NSXMLElement *useridElement = [NSXMLElement elementWithName:@"USERID" stringValue:emails[0]];
